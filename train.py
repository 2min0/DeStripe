...
from model import *
from dataloader import DeStripe_Dataset
...
parser.add_argument('--in_dir', type=str, default='./dataset/2frame.tif', help="input directory")
...
# Define model
_, _, _, h, w = next(iter(train_data))
in_features = 1
out_features = 16
...
xy_meshgrid = torch.tensor([x, y]).to(device)

model = GNN(meshgrid=xy_meshgrid,
...
for e in range(opt.epoch):
    ...
    for idx, data in enumerate(tqdm(train_loader)):
        ...
        img, fft_img, mask, _, _ = data
        img = img.to(device)

        # 3D to 2D (remove batch)
        fft_img = fft_img.squeeze()
        mask = mask.squeeze()
        output = torch.zeros(h, w, out_features, requires_grad=True).type(torch.complex128)
